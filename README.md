...
현장실습 기간과 겹쳐 프로젝트를 병행하여 진행해야 했기에 바쁜 기간이였다.  
원래 구상했던 기존 프로젝트는 실시간 모니터링 어플을 생각하지 못해, 다소 밋밋한 프로젝트였었다. 하지만  
...
경험이 많지 않았다. 그랬기 때문에 납땜을 하면서 인두기에 모듈이 닿아 타는 경우가 종종 있었지만, 모두 납땜해주었다.  
SMPS도 전원 플러그를 이용해 +와 -에 납땜을 해주었는데, 이 경우 가정용 220V를 사용했기 때문에, 자칫 잘못하면 위험하다 판단하여  
현장실습 기관 대표가 도움을 주었다. 각 모듈에 전원을 연결하다 보니 점프선이 길어지고, 많은 모듈에 공급하게 되어  
전류가 부족해져 모듈이 작동을 하지 않을까 우려스러웠지만, 다행히 모든 모듈이 작동이 되었다.  


 프로젝트를 진행하면서 아쉬웠던 점 중에 시간이 부족했다는 것이 제일 아쉬웠다.  
시간이 부족하니 프로젝트를 진행하면서 좋은 아이디어가 떠올라도 바꿀 수 있는 것이 한정적이였다.  
대표적으로 레이저 센서의 이용 방법이 있었는데, 기존의 레이저 센서 이용 방법은 단순히 섹터의 상태만 파악하는 것이였는데,  
레이저 센서의 수량을 늘려 정확도를 높이고, 택배가 목적지까지 제대로 도착하는지의 유무도 파악하자는 의견이 있었다.  
하지만 레이저 센서를 주문하고 받기까지 시간이 오래 걸렸기에 진행하지 못한 아이디어다.  


 이런 점을 통해 프로젝트를 진행하는 데에 시간을 얼마나 효율적으로 이용해야 하는지, 계획을 잘 짜야하는지 등 개발 초기 단계의 중요성을  
