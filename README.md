...
## FlowChart
![FC](https://github.com/user-attachments/assets/0ccdb4df-8743-4b31-ab4a-de88f9330db8)  
...
## 마무리
 이 프로젝트는 학교 졸업 작품 제출을 기반으로 한 프로젝트이다.  
현장실습 기간과 겹쳐 프로젝트를 병행하여 진행해야 했기에 바쁜 기간이었다.  
원래 구상했던 기존 프로젝트는 실시간 모니터링 앱을 생각하지 못해, 다소 밋밋한 프로젝트였었다. 하지만  
현장실습을 통해 익히고 있던 서버 연동 방법이 떠올라 추가하게 되었는데, 의도대로 구현이 된 것 같아 만족스럽다.  


 이 프로젝트를 진행하면서 납땜 등의 하드웨어를 제작하는 시간이 길었는데, 이 기간 동안 많은 문제가 발생했었다.  
레이저 센서 등 구매한 모듈이 납땜질이 되어있지 않아 직접 납땜을 했어야 했다. 현장실습을 다니면서 납땜질을 하는 법은 배웠지만,  
경험이 많지 않았다. 그랬기 때문에 납땜을 하면서 인두기에 모듈이 닿아 타는 경우가 종종 있었지만, 모두 납땜해 주었다.  
SMPS도 전원 플러그를 이용해 +와 -에 납땜을 해주었는데, 이 경우 가정용 220V를 사용했기 때문에, 자칫 잘못하면 위험하다 판단하여  
현장실습 기관 대표가 도움을 주었다. 각 모듈에 전원을 연결하다 보니 점퍼 케이블이 길어지고, 많은 모듈에 공급하게 되어  
전류가 부족해져 모듈이 작동을 하지 않을까 우려스러웠지만, 다행히 모든 모듈이 작동이 되었다.  


 프로젝트를 진행하면서 아쉬웠던 점 중에 시간이 부족했다는 것이 제일 아쉬웠다.  
시간이 부족하니 프로젝트를 진행하면서 좋은 아이디어가 떠올라도 바꿀 수 있는 것이 한정적이었다.  
대표적으로 레이저 센서의 이용 방법이 있었는데, 기존의 레이저 센서 이용 방법은 단순히 섹터의 상태만 파악하는 것이었는데,  
레이저 센서의 수량을 늘려 정확도를 높이고, 택배가 목적지까지 제대로 도착하는지의 유무도 파악하자는 의견이 있었다.  
하지만 레이저 센서를 주문하고 받기까지 시간이 오래 걸렸기에 진행하지 못한 아이디어다.  


 이런 점을 통해 프로젝트를 진행하는 데에 시간을 얼마나 효율적으로 이용해야 하는지, 계획을 잘 짜야 하는지 등 개발 초기 단계의 중요성을  
깨닫게 되었다. 프로젝트 마감 기간까지의 여유 기간을 충분히 조절하여 개발 초기 단계에서 구조와 흐름을 잘 짜고, 후에 개발 단계에서  
수정할 사항이 생겨도 늦지 않게 마무리할 수 있도록 프로젝트를 개발하겠다.
